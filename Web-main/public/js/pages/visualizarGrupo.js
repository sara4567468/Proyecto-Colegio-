  ...
  .then(data=>{
    if(data.tareas&&data.tareas.length>0){
      ...
      data.tareas.forEach(tarea=>{
        ...
        verEntregasBtn.onclick=function(){verEntregas(tarea.tarea_id)}
        let editBtn=document.createElement('button')
        editBtn.innerText='Editar'
        editBtn.className='submit-task-button'
        editBtn.onclick=function(){editarTarea(tarea.tarea_id)}
        let eliminarBtn=document.createElement('button')
        eliminarBtn.innerText='Eliminar'
        eliminarBtn.className='submit-task-button'
        eliminarBtn.onclick=function(){eliminarTarea(tarea.tarea_id)}
...
window.onload=function(){
  fetchEstudiantes()
  fetchTareas()
  document.getElementById('inscribirEstudianteBtn').onclick=function(){
    window.location.href='inscribirEstudiante.html'
  }
  document.getElementById('asignarTareaBtn').onclick=function(){
